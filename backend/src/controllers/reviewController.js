...
const refreshProductRating = async (productId) => {
  const [stats] = await Review.aggregate([
    { $match: { product: mongoose.Types.ObjectId.createFromHexString(String(productId)) } },
    { $group: { _id: null, rate: { $avg: '$rating' }, count: { $sum: 1 } } },
  ]);
  await Product.findByIdAndUpdate(productId, {
    rating: { rate: stats?.rate || 0, count: stats?.count || 0 },
  });
};
