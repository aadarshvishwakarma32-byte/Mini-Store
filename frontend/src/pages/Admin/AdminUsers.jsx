  ...
  return (
    <section className="adminContentPage">
      <div className="adminPageIntro">
        <p className="adminEyebrow">ACCESS CONTROL</p>
        <h1>Users & roles</h1>
        <p>Choose who can administer the store and temporarily disable access when needed.</p>
      </div>
      {loading ? (
        <div className="adminDashboardLoading">Loading users…</div>
      ) : (
            ...
            <tbody>
              {users.map((user) => (
                <tr key={user._id}>
                  ...
                  <td>
                    <button
                      className={`adminStatusButton ${user.isActive ? 'enabled' : ''}`}
                      type="button"
